...
def max_flow_value(music1, music2, mode=1, k=0.2, consider_note_distance=False,
    ...
    # Compute the sum of top k max flow connect vvalue, if exist
    
    k = 10
    if len(max_flow_connect) >= k:
        max_flow_value = sum([d[2] for d in max_flow_connect[:k]])
    else:
        max_flow_value = sum([d[2] for d in max_flow_connect])
